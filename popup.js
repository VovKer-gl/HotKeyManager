document.addEventListener('DOMContentLoaded', function () {
    ...
    const categoryData = {
        ...
        'other': [
            {id: 'frame-fixer-toggle', name: 'Enable Frame Fix Buttons (+/- 1F)', type: 'toggle'},
            {id: 'video-scrubber-toggle', name: 'Enable Wheel/Mouse Video Scrubber', type: 'toggle'},
            {id: 'invert-scrubber-toggle', name: 'Invert Scrubber Wheel Direction', type: 'toggle'},
            ...
            {id: 'qa-helper-toggle', name: 'Enable QA Report Helper', type: 'toggle'},
            {id: 'toggle-edit-mode', name: 'Toggle Edit Mode', defaultShortcut: 'M'},
            {id: 'find-event-by-clipboard', name: 'Find Event by Copied Timestamp', defaultShortcut: 'Control + F'}
        ]
    };

    function getSettingKeyFromId(id) {
        if (id === 'qa-helper-toggle') return 'qaHelperEnabled';
        if (id === 'carry-line-snap-toggle') return 'carryLineSnapEnabled';
        if (id === 'auto-confirm-toggle') return 'autoConfirmEnabled';
        if (id === 'frame-fixer-toggle') return 'frameFixerEnabled';
        if (id === 'video-scrubber-toggle') return 'videoScrubberEnabled';
        if (id === 'invert-scrubber-toggle') return 'invertScrubberEnabled';
    ...
    async function initializeDefaults(force = false) {
        ...
        for (const category in categoryData) {
            ...
            const defaultsInCategory = categoryData[category].filter(item => item.defaultShortcut).map(item => ({
                actionId: item.id,
    ...
    function createToggleItem(item, isEnabled) {
        ...
        itemDiv.innerHTML = ` <div class="event-info"><div class="event-name">${item.name}</div></div> <label class="switch"> <input type="checkbox" id="${item.id}"> <span class="slider"></span> </label> `;
    ...
    function stopRecording(shouldSave) {
        ...
        isRecording = false;
        if (shouldSave) saveCurrentHotkey(); else loadHotkeys();
        if (currentInput) {
    ...
    async function saveCurrentHotkey() {
        ...
        if (existingIndex > -1) hotkeys[existingIndex].shortcut = shortcut; else hotkeys.push({actionId, shortcut});
